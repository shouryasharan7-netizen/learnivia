...
export async function createChildProfile(formData: FormData) {
  try {
    ...
    if (!session?.user?.id) {
      return { success: false, error: "You must be signed in to manage child profiles." };
    }

    const firstName = (formData.get("firstName") as string || "").trim();
    let lastInitial = (formData.get("lastInitial") as string || "").trim().toUpperCase();
    const grade = (formData.get("grade") as string || "").trim();
    const ageRaw = formData.get("age") as string;
    const notes = (formData.get("notes") as string || "").trim();
    const preferences = (formData.get("learningPreferences") as string || "").trim();
    ...
    if (ageRaw) {
      ...
      if (isNaN(parsedAge) || parsedAge < 4 || parsedAge > 17) {
        return { success: false, error: "Student age must be between 4 and 17." };
      }
      age = parsedAge;
    }
...
export async function deleteChildProfile(childId: string) {
  try {
    const session = await auth();
    if (!session?.user?.id) {
      return { success: false, error: "You must be signed in to delete a child profile." };
    }
    ...
    if (child.parentId !== session.user.id && (session.user as any).role !== "ADMIN") {
      return { success: false, error: "Unauthorized: You can only manage your own children's profiles." };
    }
    ...
    return { success: false, error: error.message || "Failed to delete child profile." };
  }
}
