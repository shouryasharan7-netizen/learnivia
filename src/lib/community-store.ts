...
const DEFAULT_ANNOUNCEMENTS = [
  {
    ...
    authorColor: "#0E8345",
    content:
      "Welcome to the Learnivia Community! This is your space to connect with fellow learners and volunteer tutors around the globe. Join live sessions, ask questions in Homework Help, and start study circles in the channels below.",
...
const communityCache = new Map<
  string,
  { messages: CommunityMessage[]; timestamp: number }
>();
...
export async function getMessages(
  ...
  try {
    ...
    if (
      rows.length === 0 &&
      (!channel || channel === "Announcements" || channel === "Home")
    ) {
    ...
    const messages: CommunityMessage[] = rows.map((r) => {
      ...
      return {
        ...
        authorRole:
          (r.authorRole as "STUDENT" | "TUTOR" | "COMMUNITY LEAD") || "STUDENT",
...
export async function addMessage(msg: {
  ...
  const timeStr = new Date(created.createdAt).toLocaleTimeString([], {
    hour: "numeric",
    minute: "2-digit",
  });
  return {
...
export async function toggleReaction(
  ...
  try {
    ...
    const currentReactions = (existing.reactions as Record<string, number>) || {
      heart: 0,
      clap: 0,
      bulb: 0,
      fire: 0,
    ...
    currentReactions[reactionType] =
      (Number(currentReactions[reactionType]) || 0) + 1;
    ...
    const timeStr = new Date(updated.createdAt).toLocaleTimeString([], {
      hour: "numeric",
      minute: "2-digit",
    });
    return {
...
export function checkContentSafety(content: string): {
  ...
  if (phonePattern.test(content)) {
    return {
      safe: false,
      reason:
        "For member safeguarding, sharing phone numbers or personal contact info is not permitted.",
    };
  }
  ...
  if (emailPattern.test(content)) {
    return {
      safe: false,
      reason:
        "For member safeguarding, sharing email addresses is not permitted in community channels. Use the Learnivia booking system to connect with tutors.",
    };
  }
  ...
  if (socialHandlePattern.test(content)) {
    return {
      safe: false,
      reason:
        "For member safeguarding, sharing social media handles is not permitted. All tutoring happens through the Learnivia platform.",
    };
  }
  ...
  for (const phrase of contactPhrases) {
    if (lower.includes(phrase)) {
      return {
        safe: false,
        reason:
          "For member safeguarding, off-platform contact requests are not permitted. All communication happens through Learnivia.",
      };
    }
  }
  ...
  const urlPattern =
    /https?:\/\/(?!learnivia|zoom\.us|meet\.google\.com)[a-zA-Z0-9\-]+\.[a-zA-Z]{2,}/i;
  if (urlPattern.test(content)) {
    return {
      safe: false,
      reason:
        "External links are not permitted in community channels to protect member safety. Share resources by describing them in text.",
    };
  }
  ...
  for (const word of prohibited) {
    if (lower.includes(word)) {
      return {
        safe: false,
        reason:
          "Message contains language that violates classroom-safe community standards.",
      };
    }
  }
